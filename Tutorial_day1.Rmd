...
```{r load_data}
#PATH="/home/radmin/SIB_scRNA-seq_Tutorial_2018/" #Abs Path to SIB course repo in the the ext-fmi machine
PATH=""
...
datt=data.frame(Individual=sapply(strsplit(as.vector(rownames(PCA)), ".", fixed=TRUE),"[",1),
                Replicate=sapply(strsplit(as.vector(rownames(PCA)), ".", fixed=TRUE),"[",2),
                PCA=PCA[,1:2], tSNE=tSNE$Y)
```
 
Projection of replicates after correction:
 
```{r mnn3}
chart_batch_pca=ggplot(datt,aes(x=PCA.PC1,y=PCA.PC2,color=Replicate))+geom_point(size=2,alpha=0.8)
chart_batch_tsne=ggplot(datt,aes(x=tSNE.1,y=tSNE.2,color=Replicate))+geom_point(size=2,alpha=0.8)
grid.arrange(chart_batch_pca, chart_batch_tsne)
```
