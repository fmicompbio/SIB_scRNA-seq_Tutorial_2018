...
Mutual nearest neighbors (MNNs) can also be used to perform batch correction and remove unwanted variability.
[*Haghverdi et al. 2018*](https://www.ncbi.nlm.nih.gov/pubmed/29608177) implemented the method and is included as *fastMNN* / *mnnCorrect* functions in the package *scran*. The method assumes that two batches contain at least one common cell type, and that the batch effect is orthogonal to the biological differences in each batch. The fastMNN function computes corrected values in a low-dimensional subspace defined by the first 50 PCs.
 
The experiment design of *tung* data does not satisfy this assumption completely. While we have 3 replicates each of individuals A, B, and C, they do not constitute 3 batches by themselves. But we can still apply the method on individuals A, B abd C separately to remove the effect of replicates.
 
We will take top 50 principal components we computed on uncorrected data and use them as features to remove replicate effects from A, B and C.
